...
pub fn add_non_interactive(
    ...
    let target_packages = if all {
        ...
        let names: Vec<String> = package_names.split(',').map(|s| s.trim().to_string()).collect();
        ...
        return Err(anyhow::anyhow!("Must specify --package, --all, or use interactive mode"));
    };
    ...
    // Get message
    let message = message
        .ok_or_else(|| anyhow::anyhow!("Must specify --message"))?;
...
pub fn save_changeset(changeset: &Changeset) -> Result<()> {
    ...
    if !changesets_dir.exists() {
        return Err(anyhow::anyhow!("Changesets directory not found. Run {} first.", "verset init".cyan()));
    }
    ...
    println!("{} Created changeset: {}", "✓".green().bold(), filename.cyan());
    println!("\nChangeset summary:");
    ...
    for entry in fs::read_dir(changesets_dir)? {
        ...
        if path.extension().and_then(|s| s.to_str()) == Some("md") {
            ...
            if let Some(end) = content.find("---\n---") {
                ...
                let description = content[end + 8..].trim().to_string();
...
pub fn status(package_filter: Option<String>) -> Result<()> {
    ...
    println!("{} {} pending changesets\n", "Found".green().bold(), changesets.len());
    
    // Calculate version impacts
    let mut version_impacts: std::collections::HashMap<String, Vec<ChangeType>> = std::collections::HashMap::new();
...
pub fn apply(package_filter: Option<String>, dry_run: bool, no_changelog: bool) -> Result<()> {
    ...
    let mut updates: std::collections::HashMap<String, (semver::Version, Vec<ChangeType>)> = std::collections::HashMap::new();
    ...
    for (_, changeset) in &changesets {
        for package_name in &changeset.packages {
            ...
            if let Some(package) = packages.iter().find(|p| &p.name == package_name) {
                let entry = updates.entry(package_name.clone())
    ...
    for (package_name, _, new_version) in &version_updates {
        if let Some(package) = packages.iter_mut().find(|p| &p.name == package_name) {
            ...
            println!("{} Updated {} to {}", "✓".green().bold(), package_name, new_version);
        }
    }
    ...
    for (path, _) in &changesets {
        ...
        if should_remove {
            fs::remove_file(path)?;
            println!("{} Removed {}", "✓".green().bold(), path.file_name().unwrap().to_str().unwrap());
        }
    }
