...
fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Commands::Add { package, r#type, message, all } => {
            if package.is_none() && r#type.is_none() && message.is_none() && !all {
                // Interactive mode
                println!("{}", "Creating new changeset (interactive mode)...".green().bold());
        ...
        Commands::Apply { package, dry_run, no_changelog } => {
            if dry_run {
        ...
        Commands::Publish { package, dry_run, registry } => {
            if dry_run {
