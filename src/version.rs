...
fn update_toml_version(content: &str, version_path: &str, new_version: &Version) -> Result<String> {
    ...
    set_nested_toml_value(&mut value, version_path, toml::Value::String(new_version.to_string()))?;
...
fn update_json_version(content: &str, version_path: &str, new_version: &Version) -> Result<String> {
    ...
    set_nested_json_value(&mut value, version_path, serde_json::Value::String(new_version.to_string()))?;
    
    Ok(serde_json::to_string_pretty(&value)?)
}

fn set_nested_toml_value(value: &mut toml::Value, path: &str, new_value: toml::Value) -> Result<()> {
...
fn set_nested_json_value(value: &mut serde_json::Value, path: &str, new_value: serde_json::Value) -> Result<()> {
    let parts: Vec<&str> = path.split('.').collect();
    let mut current = value;
